...
# --------------------------------------------------------------------------
# Analysis of synX kmers (motif plots and basewise errors)
# --------------------------------------------------------------------------
...
for (ds in data_sets) {
  ...
  for (kmer in c(kmerGT2)) {
    dir.create(paste0('project_results/kmer_plots/', kmer))
    ...
    ggplot(data.frame(KOI@elementMetadata), aes(x=SEQUENCE, y=Error_mean, color=SEQUENCE)) +
      ...
      ylab('Error rate') +
      theme_classic() +
      ggsave(paste0('project_results/kmer_plots/', kmer, '/Error_rates', '_', kmer, '_', ds ,'.pdf'))
    ...
    melt(id.vars = c('Kmer_NT', 'Occurence', 'REF_NT'), data = kmer_tot_df %>% 
      ...
      ylab("Error rate") +
      theme_classic() +
      ggsave(paste0('project_results/kmer_plots/', kmer, '/Base_wise_error_rates_', '_', kmer, '_', ds ,'.pdf'))
    ...
    data.frame(kmer_tot_df) %>%
      ...
      ggsave(paste0('project_results/kmer_plots/', kmer, '/Motif_error_rates_', '_', kmer, '_', ds ,'.pdf'))
  }
}
