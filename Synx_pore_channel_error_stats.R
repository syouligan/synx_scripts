...
for(feature in c(synx_ranges_perf$name, "SynX")) {
  ...
  ONT_array$Row <- rep(LETTERS[1:7], each = 18)
  ONT_array$Column <- rep(1:18, 7)
  
  dir.create(paste0("project_results/Error_perPore_flongle"))
  ggplot(ONT_array, aes(x = Column, y = Row)) +
    ...
    ggsave(paste0("project_results/Error_perPore_flongle/Mean_error_rates_per_pore_", feature, ".pdf"))
}

# Minion
for(feature in c(synx_ranges_perf$name, "SynX")) {
  # Load sequencing error rates
  reads_1linebed <- read.table(paste0("project_results/ONT_DNA_Fragmentase/", feature, "_perRead/", feature, "_one_line.ONT_DNA_Fragmentase_20000.INTERSECT.perRead.tsv"), header = TRUE, sep=";", stringsAsFactors=FALSE, quote="")
  reads_1linebed$start_time <- as.POSIXct(reads_1linebed$start_time, format="%Y-%m-%dT%H:%M:%SZ")
  reads_1linebed$ch <- factor(as.character(reads_1linebed$ch))
  ...
  # Annotate with position in flow cell (row = A:P, column = 1:32)
  
  ONT_array <- data.frame("Pore" = 1:512)
  idx <- match(ONT_array$Pore, as.numeric(sum_stats$ch))
  ONT_array$ch <- sum_stats$ch [idx]
  ONT_array$Mean_error_freq <- sum_stats$Mean_error_freq  [idx]
  ONT_array$freq <- sum_stats$freq  [idx]
  ONT_array$Row <- rep(LETTERS[1:16], each = 32)
  ONT_array$Column <- rep(1:32, 16)
  
  dir.create(paste0("project_results/Error_perPore_minion"))
  ggplot(ONT_array, aes(x = Column, y = Row)) +
    geom_point(aes(fill = Mean_error_freq), shape = 21, size = 6) +
    scale_fill_gradientn(colours = hcl.colors(3, palette = 'Zissou 1', rev = FALSE), na.value = "white") +
    theme_classic() +
    scale_x_continuous(breaks=c(1:32)) +
    ggsave(paste0("project_results/Error_perPore_minion/Mean_error_rates_per_pore_", feature, ".pdf"))
}